...
【⚠️ 配额窗口口径待重评（2026-09-12 21:30）】
下方「固定重置于每天 14:26:22」的窗口口径**已被推翻**：6004 机制已定案为
**滑动窗口请求数限流**（重置 = 容量绑定请求 + 窗口长，见 find_6004_events.py 与 HANDOFF.md），
credit_ledger 的 14:26:22 边界在滑动窗语义下尚无解释（HANDOFF 下一步 2）。
本脚本当前仅作**软信号参考**（用量 vs 峰值基线的量级对比），窗口切分结论勿外推。

【原口径（待重评，勿直接采信）】
WorkBuddy 每模型每日频率限制 = 固定重置于每天 14:26:22（北京时间）的 24h 窗口，
窗口为 [14:26:22 → 次日 14:26:22)。非日历日(00:00)、非触顶后滚动 24h。
（原验证脚本 verify_reset_anchor.py 已删除——其固定锚结论同被推翻。）
