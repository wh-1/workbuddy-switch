...
export function AlignOptionsPanel({ value, onChange, preview }: Props) {
  return (
    <>
      ...
      <Row
        title="共享会话"
        hint="把当前账号的会话共享给目标账号：不占额外空间，手机端也能看到，目标已有的不会重复。单独用时有多少搬多少；和「清理旧对话」一起用时，保留范围按两个账号合并后算"
      ...
      <div className="flex items-center justify-between gap-3 rounded-md border px-3 py-2.5">
        <div className="min-w-0 flex-1">
          <div className="text-sm font-medium">清理旧对话</div>
          <div className="text-xs text-muted-foreground">
            把旧对话收起来，每个项目只留最近几条（本机文件保留、随时能恢复，Token 统计不受影响；云端那份是真删除，删了回不来）。云端上属于这个账号的也一起清，别人的不动；个别没清掉的，那条就先留在本机，下次切号再试。和「共享会话」一起用时，保留范围按两个账号合并后算
          </div>
        </div>
...
export function formatAlignReport(r: AlignDataReport): PreviewReport {
  ...
  if (f) {
    ...
    if (f.theme) {
      // 主题跟随是无条件的（真实执行也如此），除非报告明确给了 planned=false。
  ...
  if (r.slim) {
    if (r.slim.error) {
      ...
      slimGroup = { title: "清理旧对话", tone: "warn", items };
    }
  }
  ...
  if (r.slim && !r.slim.error) bits.push(`清理旧对话 ${r.slim.planned ?? 0} 条`);
