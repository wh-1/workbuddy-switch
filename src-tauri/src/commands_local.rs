...
use wb_switch_core::modules::{account, align, discover, limits};
...
pub async fn align_data(
    ...
    let opts = align::AlignOptions {
        ...
        slim_delete_cloud: slim_delete_cloud.unwrap_or(false),
...
/// GET /api/limits —— 本机模型限额快照（账号 × 模型 × 官方解锁时刻）。增量扫日志并更新索引。
#[tauri::command(rename_all = "camelCase")]
pub async fn list_model_limits(limit_days: Option<u32>) -> Value {
    let days = limit_days.unwrap_or(30);
    tauri::async_runtime::spawn_blocking(move || limits::snapshot(days))
        .await
        .unwrap_or_else(|e| json!({ "error": e.to_string() }))
}

/// POST /api/limits/refresh —— 同 list（保留语义：强制重算一次快照）。
#[tauri::command(rename_all = "camelCase")]
pub async fn refresh_model_limits(limit_days: Option<u32>) -> Value {
    let days = limit_days.unwrap_or(30);
    tauri::async_runtime::spawn_blocking(move || limits::snapshot(days))
        .await
        .unwrap_or_else(|e| json!({ "error": e.to_string() }))
