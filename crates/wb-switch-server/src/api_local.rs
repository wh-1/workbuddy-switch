...
pub fn router() -> Router {
    Router::new()
        ...
        .route("/api/limits", get(api_list_model_limits))
}

/// GET /api/limits —— 模型限额快照（webui 通道；桌面端走同名 Tauri 命令）。
async fn api_list_model_limits(
    ...
    let days = query
        ...
        .unwrap_or(30);
    json_ok(limits_local::snapshot(days))
}
