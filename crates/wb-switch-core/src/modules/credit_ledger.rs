...
//!
//! 定位：**只写不读的冷备**（2026-09-17 决策）。要看数据就直读 `.jsonl`
//! （`scripts/analysis/credit_ledger_vs_usage.py` 已含两口径对账），不进 API / UI ——
//! 上游 `/api/credits/stats` 已经回答了「某账号花了多少分」。
//!
//! **与上游 `credit_usage` 的分工**（2026-09-17 实测对账）：
//! `credit_usage` 是**余额快照差分**（只累余额下降，只在主动查分的日子有采样，
//! 实测近 30 天只覆盖 9 天），回答"大概花了多少"；本账本是**官方逐笔明细**
//! （覆盖 14~25 天，同日对齐后仍比快照高 ~9.5%），还能回答"哪个模型最烧分"。
//! 两者不是替代关系：快照法做不了按模型/按客户端的拆分，也补不回没采样的日子。
